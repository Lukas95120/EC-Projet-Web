document.addEventListener("DOMContentLoaded", () => {
    ...
    async function refreshNotificationsCount() {
        ...
        try {
            const response = await fetch(`notifications_count.php?t=${Date.now()}`, {
                cache: "no-store",
                headers: {
                    "X-Requested-With": "XMLHttpRequest"
                }
    ...
    async function refreshNotificationsPage() {
        if (!notificationsContainer || isDeletingAllNotifications) {
            return;
        }
    ...
    if (notificationsContainer) {
        ...
        setInterval(refreshNotificationsPage, 5000);
    }
    ...
    if (confirmAdminDeleteButton) {
        confirmAdminDeleteButton.addEventListener("click", () => {
    ...
    if (confirmUserStatusButton) {
        confirmUserStatusButton.addEventListener("click", () => {
            if (pendingUserStatusForm) {
                pendingUserStatusForm.submit();
            }
        });
    }
